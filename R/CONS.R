...
fct.eta2 <- function(vec,x,weights) {
  ...
  dist2.ind <- rowSums(t(t(X^2)*col.w))
    ...
    if (graph & (ncp>1)) {
        dev.new()
        par(mfrow=c(1,2))
        plot.PCA(res, choix = "var",ylim=c(-4,4),xlim=c(-4,4),title=paste("Attribute:",TI),yaxt="n",xaxt="n",cex=0.5,cex.lab=0.6)
axis(1,c(-4:4),cex.axis=0.6)
axis(2,c(-4:4),cex.axis=0.6)
        plot.PCA(res, choix = "ind", ylim=c(-4,4),xlim=c(-4,4),title="",yaxt="n",xaxt="n",cex=0.5,cex.lab=0.6)
axis(1,c(-4:4),cex.axis=0.6)
axis(2,c(-4:4),cex.axis=0.6)
    }
}
...
VTAD<-function(h,...){
  ...
  DMVA<-function(n,npp,nrr,naa,gdata1){
  varib<-gdata1
  n<-n
  nn=naa
  mm=npp*nrr
  namesmat<-matrix(,1,nn)
  for(i in 1:nn){
    namesmat[,i]<-paste0("Mat", i, ".txt")
  }
  ...
  while(cont<limn){
    for(i in 1:mm){
     for(j in 1:n){
      x[i,j]<-varib[i+cont,j]
     }
    }
  cont<-cont+mm
  }
  ana<-anames[nn-res]
  colnames(x)<-paste0("J-",ana,colnames(varib))
  write.table(x,namesmat[,nn-res])
  res<-res-1
  }
  ...
  names1<-ls(pattern = "Mat.$")

  for(i in 1:nn) {
    x<-read.table(namesmat[,i])
    print(paste("Data:",names1[i]),quote=F)
    pandoc.table(x,plain.ascii = TRUE)
    varib<-x
    m<-nrow(varib)
    n<-ncol(varib)
    ...
    outputArea <- gtext(container=g3, expand=TRUE,width = 780,height= 200)
    gr1<- ggroup(horizontal=TRUE, spacing=0, container = g3)
    b1<-glabel("Variables Times Assesor     ",container=gr1)
    font(b1) <- list(weight="bold",size= 10,family="sans",align ="left",spacing = 5)
    out <- capture.output(DMVA(n,npp,nrr,naa,gdata1))
    dispose(outputArea)
    if(length(out)>0)
      add(outputArea, out)
}
...
#------------------------------------------------------------------------------------------------------
    ...
    b1<-glabel("Assessors Times Variable     ",container=gr1)
    font(b1) <- list(weight="bold",size= 10,family="sans",align ="left",spacing = 5)
    b2<-gbutton("Loading Plots",container=gr1,handler=loadingAV)
...
# 4 -------------------------------------------------------------------------------------------
#Variables Times Assesor Consonance
VTAC<-function(h,...){
  n<-get("n",envir =mi)
  npp<-get("npp",envir =mi)
  nrr<-get("nrr",envir =mi)
  ...
  COVA<-function(n,npp,nrr,naa,gdata1){
    varib<-gdata1
   nfac<-ncol(varib)
  n<-n
  nn=naa
  ...
  names1<-ls(pattern = "Mat.$")
  assign("namesmat1",namesmat,envir =mi)
  MVAF<-0
  MCON<-matrix(,1,1)
  for(i in 1:naa) {
    x<-read.table(namesmat[,i])
...
#------------------------------------------------------------------------------------------------------
    ...
    dev.new()
    par(mfrow=c(1,2))
    matplot(MVAF1,type =c("o"),pch=20,ylim=c(0,100),col=color,xaxt="n",ylab="",cex.axis=0.5,cex.lab=0.5,cex.names=0.5,main=paste("Proportion VAF per dimension for the",nma,"PCAs"))
    axis(1,1:nrow(MVAF1),rownames(MVAF1),cex.axis=0.5)
    legend("topright",legend=paste("As",c(1:ncol(MVAF1))),pch=16,col=color,cex=0.6)
    print("Consonance Matrix",quote=F)
    ncon<-nrow(MCON)
    MCON1<-MCON[2:ncon,]
    maxco<-max(MCON1)+10
    ...
    b1<-glabel("Variables Times Assesor     ",container=gr1)
    font(b1) <- list(weight="bold",size= 10,family="sans",align ="left",spacing = 5)
    out <- capture.output(COVA(n,npp,nrr,naa,gdata1))
    dispose(outputArea)
    if(length(out)>0)
      add(outputArea, out)
}
#-------------------------------------------------------------------------------------------
 # MENUS
  ...
  imp3<-list(una=gaction("Descriptive per matrix",handler= VTAD),dos=gaction("Consonance",handler=VTAC))

  imp1<-list(Assesors.times.variable=imp2,Variables.times.assessor=imp3)

  menulistaZ<-list(u0=gaction("Parameters",handler=parm),u1=gaction("Descriptive general",handler=cd),Consonance.analysis=imp1)
  ...
  tg<-glabel("                 Instituto Tecnologico                                             Sophisticated Research                    Universidad Veracruzana             ",container=tmp1)
  font(tg) <- list(weight="bold",size= 12,family="sans",align ="center",spacing = 5)
  tg<-glabel("                 Autonomo de Mexico ",container=tmp1)
  font(tg) <- list(weight="bold",size= 12,family="sans",align ="center",spacing = 5)
  tg<-glabel("                                                                  ",container=tmp1)
  tg<-glabel("                                                                  ",container=tmp1)
  visible(w) <- TRUE
